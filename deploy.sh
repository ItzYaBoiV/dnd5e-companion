...
# Root compose + LiteLLM load-balancer each need an explicit /24. Otherwise the
# load-balancer project creates load-balancer_default from Docker's default pools
# and you hit: "all predefined address pools have been fully subnetted".
step "Finding free Docker subnets (app + LiteLLM)"
...
NODE_ENV=production
# LiteLLM on the Docker host — must NOT be localhost (that is the backend container itself)
...
# docker-compose.yml is versioned in-repo only (deploy does not rewrite it — avoids drift / duplicate lines).
...
echo ""
echo -e "  ${BOLD}Worker setup:${RESET}"
